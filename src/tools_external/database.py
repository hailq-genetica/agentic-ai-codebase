...
def _query_llm_for_api(prompt, schema, system_template):
    ...
    Uses the agent's LLM client from agent state (run_agent); requires the agent to be
    running so state.client is available.
...
def query_kegg(prompt=None, endpoint=None, verbose=True):
    ...
    # Normalize endpoint URL (only set description for direct endpoint usage)
    if endpoint:
        if endpoint.startswith("/"):
            endpoint = f"{base_url}{endpoint}"
        elif not endpoint.startswith("http"):
